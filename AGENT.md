...
### 强制使用 2 个 SubAgent 规则
每次执行任务时**必须调用至少 2 个子智能体**：
- **分工模式 A**：一个 subagent 负责探索/分析，另一个负责实施
- **分工模式 B**：一个 subagent 负责编码实现，另一个负责审查
- **协作流程**：
  1. 先启动探索/分析类 subagent 理解需求
  2. 再启动实施/编码类 subagent 进行实现
  3. 最后可启动审查类 subagent 进行代码审查

> **重要**：单个 subagent 无法完成任何实质性任务，必须成对调用！

## 协作哲学
- **共同推进**：每次交互必须推进1个具体子目标（非孤立响应）
- **旅程视角**：将对话视为一段共同的旅程，而不是一系列孤立的提示
- **长期质量**：在整个互动过程中，重点是整个对话的质量和生产力
- **耐心伙伴**：做一个有耐心、有远见的合作伙伴
...
## ⚠️ 约束条件
- **必须使用 2 个 subagent**：每次任务必须同时调用 2 个 subagent（探索+实施，或编码+审查）
- 每次最多启动 2 个 subagent
...
#### 强制 2 个 SubAgent 协作模式

**方式一：探索 + 实施**
```
1. SubAgent-1 (探索者)：分析需求、搜索现有代码、制定实现方案
2. SubAgent-2 (实施者)：基于方案进行代码实现
3. (可选) Review Agent：代码审查
```

**方式二：编码 + 审查**
```
1. SubAgent-1 (编码者)：编写/修改代码
2. SubAgent-2 (审查者)：审查代码、提出改进建议
3. SubAgent-1：根据审查意见进行修改
```
...
subagent_1:
  role: "架构分析师"
  task: "分析现有主题系统，搜索 dark mode 相关实现"
  output: "实现方案文档"
